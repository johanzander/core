"""Provides a client for interacting with the Growatt server API.

Include utility functions, enumerations, and a class `GrowattApi` to handle
authentication, data retrieval, and management of Growatt devices and plants.
"""
...
def hash_password(password):
    """Perform normal MD5, except add c if a byte of the digest is less than 10."""
...
class Timespan(IntEnum):
    """Enumeration for different timespans used in data queries.

    Attributes:
        hour (int): Represents an hourly timespan.
        day (int): Represents a daily timespan.
        month (int): Represents a monthly timespan.
...
class GrowattApi:
    """A client for interacting with the Growatt server API.

    This class provides methods to authenticate, retrieve data, and manage settings
    for Growatt devices and plants.

    Attributes:
        server_url (str): The base URL for the Growatt API.
        agent_identifier (str): The user agent identifier for API requests.
    ...
    def __init__(
        self, add_random_user_id=False, agent_identifier=None, token=None
    ) -> None:
    ...
    def __get_date_string(self, timespan=None, target_date=None):
    ...
    def plant_detail(self, plant_id, timespan, target_date=None):
        ...
        Args:
            ...
            target_date (datetime, optional): The date you are interested in. Defaults to datetime.datetime.now().
        ...
        """
        date_str = self.__get_date_string(timespan, target_date)
    ...
    def inverter_data(self, inverter_id, target_date=None):
        ...
        """
        date_str = self.__get_date_string(target_date=target_date)
        response = self.session.get(
    ...
    def tlx_energy_prod_cons(
        self, plant_id, tlx_id, timespan=Timespan.hour, target_date=None
    ):
        ...
        Args:
            ...
            tlx_id (str): The ID of the TLX inverter.
            timespan (Timespan): The ENUM value conforming to the time window you want e.g. hours from today, days, or months.
            target_date (datetime): The date you are interested in.
    ...
    def tlx_data(self, tlx_id, target_date=None):
    ...
    def mix_info(self, mix_id, plant_id=None):
        """Return high level values from Mix device.

        Args:
            mix_id (str): The serial number (device_sn) of the inverter.
            plant_id (str, optional): The ID of the plant (the mobile app uses this but it does not appear to be necessary). Defaults to None.

        Returns:
            dict: A dictionary containing high-level values from the Mix device, including:
    ...
    def mix_totals(self, mix_id, plant_id):
        """Return "Totals" values from Mix device.
    ...
    def mix_system_status(self, mix_id, plant_identifier):
        """Return the current "Status" from the Mix device.

        Args:
            mix_id (str): The serial number (device_sn) of the inverter.
            plant_identifier (str): The ID of the plant.
        ...
        Returns:
            dict: A dictionary containing the following keys:
                ...
                'pLocalLoad' -- Load consumption in kW.
                ...
                'status' -- System status - ENUM - Unknown values.
        ...
        response = self.session.post(
            ...
            params={
                ...
                "mixId": mix_id,
                "plantId": plant_identifier,
            },
        )

        return response.json().get("obj", {})

    def mix_detail(self, mix_id, plant_id, timespan=Timespan.hour, target_date=None):
        """Get Mix details for specified timespan.

        Args:
            mix_id (str): The serial number (device_sn) of the inverter.
            plant_id (str): The ID of the plant.
            timespan (Timespan): The ENUM value conforming to the time window you want e.g. hours from today, days, or months (Default Timespan.hour).
            target_date (datetime, optional): The date you are interested in. Defaults to datetime.datetime.now().

        Returns:
            dict: A chartData object where each entry is for a specific 5-minute window e.g. 00:05 and 00:10 respectively (below):
        ...
        Notes:
            It is possible to calculate the PV generation that went into charging the batteries by performing the following calculation:
    ...
    def dashboard_data(self, plant_id, timespan=Timespan.hour, target_date=None):
        ...
        Returns:
            dict: A chartData object where each entry is for a specific 5-minute window e.g. 00:05 and 00:10 respectively.
    ...
    def plant_settings(self, plant_id):
        """Return a dictionary containing the settings for the specified plant.

        Args:
            plant_id (str): The ID of the plant you want the settings of.
    ...
    def is_plant_noah_system(self, plant_id):
        ...
        Args:
            plant_id (str): The ID of the plant you want the noah devices of.
    ...
    def noah_system_status(self, serial_number):
        """Return a dictionary containing the status for the specified Noah Device.

        Args:
            serial_number (str): The serial number of the Noah device for which the status is requested.
        ...
        'obj' -- An Object containing the noah device status
            ...
            'workMode'  -- Working mode of the battery (0 = Load First, 1 = Battery First)
            'soc'   -- Statement of charge (remaining battery %)
            'associatedInvSn'   -- ???
            'batteryNum'    -- Numbers of batteries
    ...
    def noah_info(self, serial_number):
        """Return a dictionary containing the information for the specified Noah Device.

        Args:
            serial_number (str): The serial number of the Noah device for which information is requested.

        Returns:
        'msg'
        'result'    -- True or False
        'obj' -- An Object containing the noah device information
            'neoList'   -- A List containing Objects
            'unitList'  -- A Object containing currency units e.g. "Euro": "euro", "DOLLAR": "dollar"
            'noah'  -- A Object containing the following
                'time_segment'  -- A List containing Objects with configured "Operation Mode"
                    NOTE: The keys are generated numerical, the values are generated with following syntax "[workingmode (0 = Load First, 1 = Battery First)]_[starttime]_[endtime]_[output power]"
                    'time_segment': {
            ...
            'plantList' -- A List containing Objects containing the following
                'plantId'   -- The ID of the plant
    ...
    def update_plant_settings(self, plant_id, changed_settings, current_settings=None):
        """Apply settings to the plant, e.g., ID, Location, Timezone.

        Args:
            plant_id (str): The ID of the plant you wish to update the settings for.
            changed_settings (dict): A dictionary containing the settings to be changed and their values.
            current_settings (dict, optional): A dictionary containing the current settings of the plant
                (use the response from plant_settings). If None, it will be fetched automatically.

        Returns:
            dict: A response from the server stating whether the configuration was successful or not.
    ...
    def update_inverter_setting(
        ...
        Args:
            ...
            default_parameters (dict): Default set of parameters for the setting call.
            parameters (dict or list of str): Parameters to be sent to the system.
                Can be a dictionary or a list of strings (converted to a dictionary).

        Returns:
            dict: JSON response from the server indicating whether the configuration was successful.
    ...
    def update_mix_inverter_setting(self, serial_number, setting_type, parameters):
        ...
        Args:
            serial_number (str): Serial number (device_sn) of the inverter.
            setting_type (str): The type of setting to be configured.
            parameters (dict or list of str): Parameters to be sent to the system.
                Can be a dictionary or a list of strings (converted to a dictionary).
    ...
    def update_tlx_inverter_time_segment(
        ...
        """Update the time segment settings for a TLX hybrid inverter.

        Args:
            serial_number (str): Serial number (device_sn) of the inverter.
            segment_id (int): ID of the time segment to be updated.
            batt_mode (int): Battery mode (0 = Load First, 1 = Battery First, 2 = Grid First).
            start_time (datetime.time): Start time of the segment.
            end_time (datetime.time): End time of the segment.
            enabled (bool): Whether the segment is enabled.

        Returns:
            dict: JSON response from the server whether the configuration was successful.
    ...
    def update_tlx_inverter_setting(self, serial_number, setting_type, parameter):
        ...
        Args:
            serial_number (str): Serial number (device_sn) of the inverter.
            setting_type (str): Setting to be configured.
            parameter (str, dict, or list): Parameter(s) to be sent to the system.
                Can be a single value, a dictionary, or a list of strings.
    ...
    def update_noah_settings(self, serial_number, setting_type, parameters):
        """Apply settings for specified noah device based on serial number.

        See README for known working settings.

        Args:
            serial_number (str): Serial number (device_sn) of the noah device.
            setting_type (str): The type of setting to be configured.
            parameters (dict or list of str): Parameters to be sent to the system.
                Can be a dictionary or a list of strings (converted to a dictionary).
    ...
    def plant_details_v1(self, plant_id):
        ...
        if not self.v1_api_enabled:
            warnings.warn(
                "V1 API is not enabled. This method requires an API token.",
                RuntimeWarning,
            )
            return {"error_code": 1, "error_msg": "API token required", "data": None}

        response = self.session.get(
            self.get_v1_url("plant/details"), params={"plant_id": plant_id}
        )
    ...
    def plant_energy_overview_v1(self, plant_id):
        ...
        response = self.session.get(
            self.get_v1_url("plant/data"), params={"plant_id": plant_id}
        )

        return response.json()

    def plant_energy_history_v1(
        self,
        plant_id,
        start_date=None,
        end_date=None,
    ...
    def min_detail(self, device_sn):
        ...
        response = self.session.get(
            self.get_v1_url("device/tlx/tlx_data_info"), params={"device_sn": device_sn}
        )
    ...
    def min_settings(self, device_sn):
        ...
        response = self.session.get(
            self.get_v1_url("device/tlx/tlx_set_info"), params={"device_sn": device_sn}
        )
    ...
    def min_write_parameter(self, device_sn, parameter_id, parameter_values=None):
        ...
        # Send the request
        response = self.session.post(self.get_v1_url("tlxSet"), data=request_data)
    ...
    def min_write_time_segment(
        ...
        all_params = {"tlx_sn": device_sn, "type": f"time_segment{segment_id}"}
        ...
        # Send the request
        response = self.session.post(self.get_v1_url("tlxSet"), data=all_params)
    ...
    def min_read_time_segments(self, device_sn, settings_data=None):
        ...
        elif "error_code" in settings_data and "data" in settings_data:
