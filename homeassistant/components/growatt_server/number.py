    ...
    async def async_set_native_value(self, value: float) -> None:
        ...
        try:
            ...
            if auth_type == AUTH_API_TOKEN:
                ...
                _LOGGER.debug(
                    "Set parameter using V1 API: %s to value: %s, res: %s",
                    self.entity_description.api_key,
                    value,
                    res,
                )
                ...
                if res.get("error_code", 1) == 0:
                    ...
                    _LOGGER.error(
                        "Set parameter: %s to value: %s failed with error: %s",
                        self.entity_description.key,
                        value,
                        res.get("error_msg", "Unknown error"),
                    )
                    raise HomeAssistantError(
                        f"Failed to set parameter {self.entity_description.key} to value {value}: {res.get('error_msg')}"
                    )
            else:
                # Traditional API
                res = await self.hass.async_add_executor_job(
